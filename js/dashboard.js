(function($) {
  'use strict';
  $(function() {
    if ($("#total-sales-chart").length) { // total sales chart
      function getTotalSalesChart() {
        fetch('https://inlupp-fa.azurewebsites.net/api/total-sales-chart')
        .then(res => res.json())              
        .then(data => {
    
            let salesGraph = data.labels;
            localStorage.setItem('totalSalesChart1', salesGraph);
    
            let salesGraphData1 = data.datasets[0].data;
            localStorage.setItem('totalSalesChart2', salesGraphData1);

            let salesGraphData2 = data.datasets[1].data;
            localStorage.setItem('totalSalesChart3', salesGraphData2);
    ...
    let salesGraphArray = [];
    salesGraphArray = localStorage.getItem('totalSalesChart1').split(",");

    let salesGraphDataArray1 = [];
    salesGraphDataArray1 = localStorage.getItem('totalSalesChart2').split(",");

    let salesGraphDataArray2 = [];
    salesGraphDataArray2 = localStorage.getItem('totalSalesChart3').split(",");
      var areaData = {
        labels: salesGraphArray,
        datasets: [
          {
            data: salesGraphDataArray1,
            backgroundColor: [
              'rgba(61, 165, 244, .0)'
            ],
            borderColor: [
    ...
    if ($("#users-chart").length) { // users-chart
      var areaData = {
        labels: usersGraphArray,
        datasets: [{
            data: usersGraphDataArray,
            backgroundColor: [
              '#e0fff4'
            ],
            borderWidth: 2,
    ...
    if ($("#projects-chart").length) { // projects chart
      var areaData = {
        labels: projectsGraphArray,
        datasets: [{
            data: projectsGraphDataArray,
            backgroundColor: [
              '#e5f2ff'
            ],
            borderWidth: 2,
    ...
    if ($("#distribution-chart").length) {
      var areaData = {
        labels: updatesGraphArray,
        datasets: [{
            data: updatesGraphDataArray,
            backgroundColor: [
    ...
    if ($("#sale-report-chart").length) { // sale report
      var CurrentChartCanvas = $("#sale-report-chart").get(0).getContext("2d");
      var CurrentChart = new Chart(CurrentChartCanvas, {
        type: 'bar',
        data: {
          labels: salesGraphMonthsArray,
          datasets: [{
              label: salesGraphLabelArray,
              data: salesGraphDataArray,
              backgroundColor: salesGraphColorArray
            }
          ]
        },
        options: {
